...
def make_dataset(directory, opt, erode_seg=True):
    ...
    war = glob.glob(osp.join(directory, opt + '_war/*/*.jpg'))
    war = sorted(war)
    #print('changed')
    ...
    if erode_seg:
        ...
        return list(zip(img, skg, seg , eroded_seg, extended_txt, war))
    else:
        #return list(zip(img, skg, seg, extended_txt, war))
        return list(zip(img, skg, war, seg))
...
class ImageFolder(data.Dataset):
    def __init__(self, opt, root, transform=None, target_transform=None,
                 loader=default_loader, erode_seg=False, sampler=None):
    ...
    def __getitem__(self, index):
        ...
        if self.erode_seg:
            img_path, skg_path, seg_path, eroded_seg_path, txt_path, war_path = self.imgs[index]
        else:
            #img_path, skg_path, seg_path, txt_path, war_path = self.imgs[index]
            img_path, skg_path, war_path, seg_path = self.imgs[index]
        
        img = self.loader(img_path)
        skg = self.loader(skg_path)
        seg = self.loader(seg_path)
        #txt = self.loader(txt_path)
        war = self.loader(war_path)
        ...
        if self.transform is not None:
            if self.erode_seg:
                img, skg, war,seg = self.transform([img, skg, war])
            else:
                #img, skg, seg, txt, war = self.transform([img, skg, seg, txt, war])
                #img, skg, war = self.transform([img, skg, war])
                img = self.transform(img)
                skg = self.transform(skg)
                war = self.transform(war)
                seg = self.transform(seg)
                #eroded_seg = seg

        #return img, skg, seg, eroded_seg, txt, war
        return img, skg, war, seg
