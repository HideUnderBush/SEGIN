...
class generator(nn.Module):
    ...
    def __init__(self, d=64):
        ...
        self.conv6 = nn.Conv2d(d * 8, d * 8, 3, 1, 1)
        ...
        self.non_local_1024 = NONLocalBlock2D(1024)
        self.non_local_512 = NONLocalBlock2D(512)

        # Unet decoder
        self.up = nn.Upsample(scale_factor = 2, mode = 'nearest')
        #self.pad = nn.ReflectionPad2d(1)
        self.pad = nn.ReplicationPad2d(1)
        #self.deconv1 = nn.ConvTranspose2d(d * 8, d * 8, 3, 1, 1)
        self.upconv1 = nn.Conv2d(d * 8, d * 8, 3, 1, 1)
        self.deconv1_bn = nn.BatchNorm2d(d * 8)
        #self.deconv2 = nn.ConvTranspose2d(d * 8 * 2, d * 8, 3, 1, 1)
        self.upconv2 = nn.Conv2d(d * 8 * 2, d * 8, 3, 1, 1)
        self.deconv2_bn = nn.BatchNorm2d(d * 8)
        #self.deconv3 = nn.ConvTranspose2d(d * 8 * 2, d * 4, 4, 2, 1)
        self.upconv3 = nn.Conv2d(d * 8, d * 4, 3, 1, 1) # need upsample here
        self.deconv3_bn = nn.BatchNorm2d(d * 4)
        #self.deconv4 = nn.ConvTranspose2d(d * 8, d * 4, 4, 2, 1)
        self.upconv4 = nn.Conv2d(d * 8, d * 4, 3, 1, 1) # need upsample here
        self.deconv4_bn = nn.BatchNorm2d(d * 4)
        #self.deconv5 = nn.ConvTranspose2d(d * 8, d * 2, 4, 2, 1)
        self.upconv5 = nn.Conv2d(d * 8, d * 2, 3, 1, 1) # need upsample here
        self.deconv5_bn = nn.BatchNorm2d(d * 2)
        #self.deconv6 = nn.ConvTranspose2d(d * 4, d, 4, 2, 1)
        self.upconv6 = nn.Conv2d(d * 4, d, 3, 1, 1) # need upsample here
        self.deconv6_bn = nn.BatchNorm2d(d)
        #self.deconv7 = nn.ConvTranspose2d(d * 2, 3, 4, 2, 1)
        self.upconv7 = nn.Conv2d(d * 2, 3, 3, 1, 1) # need upsample here


        self.deconv1_seg = nn.ConvTranspose2d(d * 8, d * 8, 4, 2, 1)
        self.deconv1_bn_seg = nn.BatchNorm2d(d * 8)
        self.deconv2_seg = nn.ConvTranspose2d(d * 8, d * 4, 4, 2, 1)
        self.deconv2_bn_seg= nn.BatchNorm2d(d * 4)
        self.deconv3_seg = nn.ConvTranspose2d(d * 4, d * 4, 4, 2, 1)
        self.deconv3_bn_seg = nn.BatchNorm2d(d * 4)
        self.deconv4_seg = nn.ConvTranspose2d(d * 4, d * 2, 4, 2, 1)
        self.deconv4_bn_seg = nn.BatchNorm2d(d * 2)
        self.deconv5_seg = nn.ConvTranspose2d(d * 2, d, 4, 2, 1)
        self.deconv5_bn_seg = nn.BatchNorm2d(d)
        self.deconv6_seg = nn.ConvTranspose2d(d, 1, 3, 1, 1)
    ...
    def forward(self, input):
        ...
        e6 = self.non_local_512(e6)
        e6_res = self.res_block(e6)
        e6_res = self.res_block(e6_res)
        e6_res = self.res_block(e6_res)
        e6_res = self.non_local_512(e6_res) 
        e6_res = self.res_block(e6_res)
        e6_res = self.res_block(e6_res)
        e6_res = self.res_block(e6_res)
        e6_nonlocal = self.non_local_512(e6_res)

        e6 = e6_nonlocal

        # Generate img results
        #d1 = F.dropout(self.deconv1_bn(self.deconv1(F.relu(e6))), 0.5, training=True)
        d1 = F.dropout(self.deconv1_bn(self.upconv1(F.relu(e6))), 0.5, training=True)
        d1 = torch.cat([d1, e5], 1)
        d1 = self.non_local_1024(d1)
        #d2 = F.dropout(self.deconv2_bn(self.deconv2(F.relu(d1))), 0.5, training=True)
        d2 = F.dropout(self.deconv2_bn(self.upconv2(F.relu(d1))), 0.5, training=True)
        #d2 = torch.cat([d2, e5], 1)
        d2 = self.non_local_512(d2)
        #d3 = F.dropout(self.deconv3_bn(self.deconv3(F.relu(d2))), 0.5, training=True)
        d3 = F.dropout(self.deconv3_bn(self.upconv3(self.up(F.relu(d2)))), 0.5, training=True)
        d3 = torch.cat([d3, e4], 1)
        d3 = self.non_local_512(d3)
        #d4 = self.deconv4_bn(self.deconv4(F.relu(d3)))
        d4 = self.deconv4_bn(self.upconv4(self.up(F.relu(d3))))
        d4 = torch.cat([d4, e3], 1)
        d4 = self.non_local_512(d4) # remove in future
        #d5 = self.deconv5_bn(self.deconv5(F.relu(d4)))
        d5 = self.deconv5_bn(self.upconv5(self.up(F.relu(d4))))
        ####d5 = self.non_local_512(d5) out of memory
        d5 = torch.cat([d5, e2], 1)
        #d6 = self.deconv6_bn(self.deconv6(F.relu(d5)))
        d6 = self.deconv6_bn(self.upconv6(self.up(F.relu(d5))))
        d6 = torch.cat([d6, e1], 1)
        #d7 = self.deconv7(F.relu(d6))
        d7 = self.upconv7(self.up(F.relu(d6)))

        #print(d7.shape)
        o = F.tanh(d7)

        # Generate seg results
        d1_seg = F.dropout(self.deconv1_bn_seg(self.deconv1_seg(F.relu(e6))), 0.5, training=True)
        d2_seg = F.dropout(self.deconv2_bn_seg(self.deconv2_seg(F.relu(d1_seg))), 0.5, training=True)
        d3_seg = F.dropout(self.deconv3_bn_seg(self.deconv3_seg(F.relu(d2_seg))), 0.5, training=True)
        d4_seg = self.deconv4_bn_seg(self.deconv4_seg(F.relu(d3_seg)))
        d5_seg = self.deconv5_bn_seg(self.deconv5_seg(F.relu(d4_seg)))
        d6_seg = self.deconv6_seg(F.relu(d5_seg))
        o_seg = F.tanh(d6_seg)
...
class refine_net(nn.Module):
    def __init__(self, d=32):
        ...
        self.conv3_bn = nn.BatchNorm2d(d * 4)
        self.conv4 = nn.Conv2d(d * 4, d * 4, 3, 1, 1) 
        self.conv4_bn = nn.BatchNorm2d(d * 4)
        self.conv5 = nn.Conv2d(d * 4, d * 2, 3, 1, 1) 
        self.conv5_bn = nn.BatchNorm2d(d * 2)
        self.conv6 = nn.Conv2d(d * 2, d, 3, 1, 1) 
        self.conv6_bn = nn.BatchNorm2d(d)
        self.conv7 = nn.Conv2d(d, 3, 3, 1, 1) 

    def forward(self, input):
        x = self.conv1(input)
        x = self.conv2_bn(self.conv2(F.relu(x)))
        x = self.conv3_bn(self.conv3(F.relu(x)))
        x = self.conv4_bn(self.conv4(F.relu(x)))
        x = self.conv5_bn(self.conv5(F.relu(x)))
        x = self.conv6_bn(self.conv6(F.relu(x)))
        #x = F.tanh(self.conv7(F.relu(x)))
...
class discriminator_snIns(nn.Module):
    ...
    def __init__(self, d=64):
        ...
        #self.conv3 = SpectralNorm(nn.Conv2d(d, d * 2, 4, 2, 1))
        ...
        #self.conv5 = SpectralNorm(nn.Conv2d(d * 2, d * 4, 4, 2, 1))
        ...
        self.fc1 = SpectralNorm(nn.Linear(d * 8, d * 8 * 2))
        self.fc2 = SpectralNorm(nn.Linear(d * 8 * 2, d * 8 * 4))
        self.fc3 = SpectralNorm(nn.Linear(d * 8 * 4, d * 8 * 4))
        self.fc4 = SpectralNorm(nn.Linear(4 * 4 * d * 8, 1))
        #self.fc = SpectralNorm(nn.Linear(3 * 3 * d * 8, 1))
    ...
    def forward(self, input_):
        ...
        x = self.fc2(x.view(-1, 512 * 2))
        x = self.fc3(x.view(-1, 512 * 4))
        #print(x.shape)
        

        #return F.sigmoid(self.fc(x.view(-1, 4 * 4 * 512))) 
        return F.sigmoid(self.fc4(x.view(-1, 4 * 4 * 512))) 
        #return F.sigmoid(self.fc(x.view(-1, 3 * 3 * 512))) 
